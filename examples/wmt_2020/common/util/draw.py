...
def draw_scatterplot(data_frame, real_column, prediction_column, path, topic):
    ...
    plt.figure()
    ax = data_frame.plot(kind='scatter', x='id', y=real_column, color='DarkBlue', label='true (sorted)', title=topic)
    ax = data_frame.plot(kind='scatter', x='id', y=prediction_column, color='orange', label='predicted',
                         ax=ax)
    ax.text(0.5 * data_frame.shape[0],
            min(min(data_frame[real_column].tolist()), min(data_frame[prediction_column].tolist())), textstr,
            fontsize=10)
    ax.set_ylabel('TQ score')
